...
def _stream_details(streams: list[dict]) -> list[dict]:
    """Normalize every FFprobe stream while retaining legacy first-track summaries."""
    details: list[dict] = []
    for ordinal, stream in enumerate(streams):
        tags = stream.get("tags") or {}
        disposition = stream.get("disposition") or {}
        title = str(tags.get("title") or "").strip()
        stream_type = str(stream.get("codec_type") or "unknown").strip().lower() or "unknown"
        ...
        index = _number(stream.get("index"), int, ordinal)
        details.append({
            "index": index,
            "type": stream_type,
            ...
            "language": language,
            "title": title,
            "channels": _number(stream.get("channels"), int) if stream_type == "audio" else None,
            ...
            "disposition": {
                str(key): int(bool(value)) for key, value in disposition.items()
            },
        })
    return details
