...
def build_router(ctx: RouteContext):
    ...
    conditions = ctx.live("conditions")
    conn = ctx.live("conn")
    count = ctx.live("count")
    credit_conditions = ctx.live("credit_conditions")
    credit_role = ctx.live("credit_role")
    csv = ctx.live("csv")
    ...
    episode_id = ctx.live("episode_id")
    episode_ids = ctx.live("episode_ids")
    episodes = ctx.live("episodes")
    exc = ctx.live("exc")
    favorite = ctx.live("favorite")
    favorite_episodes = ctx.live("favorite_episodes")
    favorite_return_path = ctx.live("favorite_return_path")
    favorite_status = ctx.live("favorite_status")
    favorite_titles = ctx.live("favorite_titles")
    field = ctx.live("field")
    ...
    form = ctx.live("form")
    fuzzy_credit_sql = ctx.live("fuzzy_credit_sql")
    fuzzy_names = ctx.live("fuzzy_names")
    fuzzy_people = ctx.live("fuzzy_people")
    gap_status = ctx.live("gap_status")
    gaps = ctx.live("gaps")
    genre = ctx.live("genre")
    genre_options = ctx.live("genre_options")
    io = ctx.live("io")
    item = ctx.live("item")
    json = ctx.live("json")
    key = ctx.live("key")
    kind = ctx.live("kind")
    kind_sql = ctx.live("kind_sql")
    letter = ctx.live("letter")
    library_export_rows = ctx.live("library_export_rows")
    match = ctx.live("match")
    match_status = ctx.live("match_status")
    matched_condition = ctx.live("matched_condition")
    media_type = ctx.live("media_type")
    metadata_options = ctx.live("metadata_options")
    name = ctx.live("name")
    normalized = ctx.live("normalized")
    normalized_letter = ctx.live("normalized_letter")
    note = ctx.live("note")
    option_condition = ctx.live("option_condition")
    option_conditions = ctx.live("option_conditions")
    option_params = ctx.live("option_params")
    output = ctx.live("output")
    params = ctx.live("params")
    people = ctx.live("people")
    people_rows = ctx.live("people_rows")
    person = ctx.live("person")
    person_id = ctx.live("person_id")
    person_name = ctx.live("person_name")
    ...
    record_search = ctx.live("record_search")
    redirect = ctx.live("redirect")
    request = ctx.live("request")
    result = ctx.live("result")
    return_to = ctx.live("return_to")
    role = ctx.live("role")
    roles = ctx.live("roles")
    root = ctx.live("root")
    root_id = ctx.live("root_id")
    root_options = ctx.live("root_options")
    row = ctx.live("row")
    rows = ctx.live("rows")
    seen = ctx.live("seen")
    selected = ctx.live("selected")
    selected_person = ctx.live("selected_person")
    sort = ctx.live("sort")
    sort_key = ctx.live("sort_key")
    sort_sql = ctx.live("sort_sql")
    sqlite3 = ctx.live("sqlite3")
    stamp = ctx.live("stamp")
    suggestion_type = ctx.live("suggestion_type")
    suggestions = ctx.live("suggestions")
    tag = ctx.live("tag")
    tag_id = ctx.live("tag_id")
    tag_options = ctx.live("tag_options")
    tag_rows = ctx.live("tag_rows")
    tags = ctx.live("tags")
    ...
    title_order = ctx.live("title_order")
    title_return_path = ctx.live("title_return_path")
    title_rows = ctx.live("title_rows")
    title_sort_base = ctx.live("title_sort_base")
    title_sort_sql = ctx.live("title_sort_sql")
    title_type = ctx.live("title_type")
    title_type_options = ctx.live("title_type_options")
