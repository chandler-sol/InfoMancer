...
def build_router(ctx: RouteContext):
    ...
    failure = ctx.live("failure")
    failures = ctx.live("failures")
    file_id = ctx.live("file_id")
    file_ids = ctx.live("file_ids")
    file_row = ctx.live("file_row")
    frequency = ctx.live("frequency")
    genre_job = ctx.live("genre_job")
    hash_counts = ctx.live("hash_counts")
    hash_job = ctx.live("hash_job")
    ids = ctx.live("ids")
    imdb_genre_job = ctx.live("imdb_genre_job")
    imdb_genre_lock = ctx.live("imdb_genre_lock")
    job = ctx.live("job")
    label = ctx.live("label")
    match_job = ctx.live("match_job")
    match_label = ctx.live("match_label")
    match_mode = ctx.live("match_mode")
    ...
    title_scans = ctx.live("title_scans")
