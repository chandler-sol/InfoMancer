...
def build_router(ctx: RouteContext):
    ...
    all_candidates = ctx.live("all_candidates")
    ...
    applied = ctx.live("applied")
    available = ctx.live("available")
    cached_count = ctx.live("cached_count")
    candidate = ctx.live("candidate")
    candidates = ctx.live("candidates")
    category = ctx.live("category")
    changed = ctx.live("changed")
    check_source_health = ctx.live("check_source_health")
    cleanup_history = ctx.live("cleanup_history")
    cleanup_status = ctx.live("cleanup_status")
    confidence = ctx.live("confidence")
    confirm = ctx.live("confirm")
    conn = ctx.live("conn")
    context = ctx.live("context")
    counts = ctx.live("counts")
    critical_weight = ctx.live("critical_weight")
    db = ctx.live("db")
    decision = ctx.live("decision")
    destination = ctx.live("destination")
    detect_outliers = ctx.live("detect_outliers")
    different = ctx.live("different")
    direct_selection = ctx.live("direct_selection")
    dismissed = ctx.live("dismissed")
    duplicate_impact = ctx.live("duplicate_impact")
    duplicate_opportunity = ctx.live("duplicate_opportunity")
    ...
    dynamic_range = ctx.live("dynamic_range")
    evidence = ctx.live("evidence")
    exact = ctx.live("exact")
    exc = ctx.live("exc")
    failed = ctx.live("failed")
    feedback_id = ctx.live("feedback_id")
    file_a_id = ctx.live("file_a_id")
    file_b_id = ctx.live("file_b_id")
    file_id = ctx.live("file_id")
    file_ids = ctx.live("file_ids")
    finding_count = ctx.live("finding_count")
    finding_id = ctx.live("finding_id")
    findings = ctx.live("findings")
    healthy = ctx.live("healthy")
    identity_warning_threshold = ctx.live("identity_warning_threshold")
    ids = ctx.live("ids")
    index = ctx.live("index")
    information_weight = ctx.live("information_weight")
    item = ctx.live("item")
    job = ctx.live("job")
    json = ctx.live("json")
    label = ctx.live("label")
    labels = ctx.live("labels")
    left = ctx.live("left")
    matches = ctx.live("matches")
    ...
    messages = ctx.live("messages")
    mie = ctx.live("mie")
    minimum_audio_channels = ctx.live("minimum_audio_channels")
    minimum_bitrate_mbps = ctx.live("minimum_bitrate_mbps")
    minimum_height = ctx.live("minimum_height")
    minimum_width = ctx.live("minimum_width")
    mode = ctx.live("mode")
    movie_id = ctx.live("movie_id")
    movie_match_job = ctx.live("movie_match_job")
    movie_match_lock = ctx.live("movie_match_lock")
    no_result_count = ctx.live("no_result_count")
    note = ctx.live("note")
    offset = ctx.live("offset")
    pairs = ctx.live("pairs")
    parsed = ctx.live("parsed")
    part = ctx.live("part")
    path = ctx.live("path")
    placeholders = ctx.live("placeholders")
    preferred_containers = ctx.live("preferred_containers")
    preferred_video_codecs = ctx.live("preferred_video_codecs")
    preview = ctx.live("preview")
    q = ctx.live("q")
    query = ctx.live("query")
    re = ctx.live("re")
    reason = ctx.live("reason")
    record_event = ctx.live("record_event")
    redirect = ctx.live("redirect")
    refresh = ctx.live("refresh")
    remediation_context = ctx.live("remediation_context")
    report = ctx.live("report")
    request = ctx.live("request")
    result = ctx.live("result")
    results = ctx.live("results")
    retention = ctx.live("retention")
    review = ctx.live("review")
    right = ctx.live("right")
    root_id = ctx.live("root_id")
    roots = ctx.live("roots")
    row = ctx.live("row")
    rows = ctx.live("rows")
    run_bulk_verification = ctx.live("run_bulk_verification")
    ...
    run_verification = ctx.live("run_verification")
    scan_jobs = ctx.live("scan_jobs")
    scan_lock = ctx.live("scan_lock")
    scope = ctx.live("scope")
    selected = ctx.live("selected")
    selected_ids = ctx.live("selected_ids")
    selected_scope = ctx.live("selected_scope")
    series_id = ctx.live("series_id")
    severity = ctx.live("severity")
    sort = ctx.live("sort")
    source = ctx.live("source")
    source_options = ctx.live("source_options")
    source_stale_hours = ctx.live("source_stale_hours")
    ...
    suggestion_rows = ctx.live("suggestion_rows")
    suggestions = ctx.live("suggestions")
    summary = ctx.live("summary")
