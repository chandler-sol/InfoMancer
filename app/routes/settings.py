...
def build_router(ctx: RouteContext):
    ...
    archive = ctx.live("archive")
    auth_error_response = ctx.live("auth_error_response")
    available = ctx.live("available")
    backup_name = ctx.live("backup_name")
    backups = ctx.live("backups")
    base64 = ctx.live("base64")
    candidate = ctx.live("candidate")
    candidate_path = ctx.live("candidate_path")
    category = ctx.live("category")
    changed = ctx.live("changed")
    changes = ctx.live("changes")
    check_source_health = ctx.live("check_source_health")
    chunk = ctx.live("chunk")
    cleaned = ctx.live("cleaned")
    confirm = ctx.live("confirm")
    conn = ctx.live("conn")
    content = ctx.live("content")
    create_database_backup = ctx.live("create_database_backup")
    csv = ctx.live("csv")
    csv_safe_row = ctx.live("csv_safe_row")
    current = ctx.live("current")
    cursor = ctx.live("cursor")
    database_file = ctx.live("database_file")
    datetime = ctx.live("datetime")
    db = ctx.live("db")
    default_cover_size = ctx.live("default_cover_size")
    default_library_view = ctx.live("default_library_view")
    destination = ctx.live("destination")
    detail = ctx.live("detail")
    encoded = ctx.live("encoded")
    encoded_settings = ctx.live("encoded_settings")
    engagement = ctx.live("engagement")
    event_log = ctx.live("event_log")
    events = ctx.live("events")
    exc = ctx.live("exc")
    exit_for_container_restart = ctx.live("exit_for_container_restart")
    explanation = ctx.live("explanation")
    failures = ctx.live("failures")
    filename = ctx.live("filename")
    hash_immediate_limit = ctx.live("hash_immediate_limit")
    hash_io_intensity = ctx.live("hash_io_intensity")
    hash_mode = ctx.live("hash_mode")
    hash_pause_for_activity = ctx.live("hash_pause_for_activity")
    hash_schedule_day = ctx.live("hash_schedule_day")
    hash_schedule_frequency = ctx.live("hash_schedule_frequency")
    hash_schedule_time = ctx.live("hash_schedule_time")
    ids = ctx.live("ids")
    imported = ctx.live("imported")
    ...
    name = ctx.live("name")
    os = ctx.live("os")
    output = ctx.live("output")
    path = ctx.live("path")
    payload = ctx.live("payload")
    ...
    release = ctx.live("release")
    release_version_key = ctx.live("release_version_key")
    releases = ctx.live("releases")
    render_settings = ctx.live("render_settings")
    repository = ctx.live("repository")
    request = ctx.live("request")
    request_headers = ctx.live("request_headers")
    resolve_backup = ctx.live("resolve_backup")
    resolved = ctx.live("resolved")
    response = ctx.live("response")
    ...
    safety = ctx.live("safety")
    scan_after = ctx.live("scan_after")
