...
def _ensure_runtime_streams(data_dir: Path) -> None:
    ...
    stream = (log_dir / "desktop-core.log").open("a", encoding="utf-8", buffering=1)
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] InfoMancer desktop core diagnostics started.", flush=True)
...
def _start_onefile_parent_watchdog() -> None:
    ...
    threading.Thread(target=watch, name="infomancer-desktop-parent-watchdog", daemon=True).start()
...
def create_recovery_package(data_dir: Path, output: Path) -> None:
    ...
    if _inside(output, data_dir):
        raise RuntimeError("Choose a recovery destination outside InfoMancer's application-data folder.")
...
def main() -> int:
    ...
    database = data_dir / "infomancer.db"
    bootstrap_token = (args.bootstrap_token or os.getenv("INFOMANCER_BOOTSTRAP_TOKEN", "")).strip()
