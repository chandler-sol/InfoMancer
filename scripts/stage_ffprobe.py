...
VERSION = "6.1.1"
RELEASE_BASE = "https://github.com/eugeneware/ffmpeg-static/releases/download/b6.1.1"
...
def _download(url: str) -> bytes:
    request = urllib.request.Request(url, headers={"User-Agent": "InfoMancer-native-packaging/0.9"})
...
def _write_notice(output: Path) -> None:
    (output / "FFPROBE_NOTICE.txt").write_text(
        "InfoMancer native desktop packages include FFprobe from FFmpeg for local media inspection.\n\n"
        f"Bundled build: FFmpeg/FFprobe {VERSION}\n"
        f"Binary source: {RELEASE_BASE}\n"
        "Upstream project: https://ffmpeg.org/\n\n"
        "The accompanying FFPROBE_LICENSE.txt is the license distributed with the pinned binary build. "
        "Review third-party distribution obligations before publishing a production release.\n",
        encoding="utf-8",
    )
...
def stage(output: Path) -> Path:
    ...
    if reused:
        print(f"Reused verified FFprobe {VERSION} at {reused}")
        return reused
    slug = asset["slug"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage the pinned FFprobe binary used by native InfoMancer packages")
