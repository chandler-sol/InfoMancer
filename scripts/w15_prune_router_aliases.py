...
for module in MODULES:
    ...
    builder = builders[0]
    required = descendant_free_names(builder)
    # Type annotations, decorator arguments, and default expressions on nested
    # route functions are evaluated while build_router runs, so keep aliases that
    # the builder itself references too.
    required.update(
        symbol.get_name()
