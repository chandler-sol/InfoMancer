...
class ActivityAndBulkFeedbackTests(unittest.TestCase):
    ...
    def test_bulk_apply_override_keeps_shell_interactive_without_keepalive(self):
        ...
        self.assertIn("Applying metadata for ${total || fallbackTotal} ${noun}", feedback)
        ...
        self.assertIn("const formData = new FormData(reviewForm)", feedback)
        self.assertIn("body: formData", feedback)
        self.assertIn("formData.set('apply_job_id', jobId)", feedback)
        ...
        self.assertIn("startApplyProgressPolling(jobId, count, noun)", feedback)
