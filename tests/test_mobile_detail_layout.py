...
class MobileDetailLayoutTests(unittest.TestCase):
    ...
    def test_phone_hero_keeps_poster_and_identity_in_one_row(self):
        css = (ROOT / "app/static/mobile.css").read_text(encoding="utf-8")
        detail_section = css.split("from mobile-detail.css", 1)[1]
        ...
        self.assertIn("grid-column: 1 / -1", detail_section)
        self.assertNotIn("grid-template-columns: 1fr;", detail_section)
    ...
    def test_mobile_detail_styles_load_after_general_mobile_polish(self):
        ...
        # The consolidated stylesheet preserves the original bootstrap cascade:
        # general polish first, then header chrome, then the detail dossier rules.
        self.assertLess(
            css.index("from final-mobile-polish.css"),
            css.index("from mobile-detail.css"),
        )
