...
class ReviewWorkspaceContractTests(unittest.TestCase):
    def test_review_workspace_and_w4_assets_are_wired(self):
        ...
        ui = (ROOT / "app" / "static" / "workspace-ui-core.js").read_text(encoding="utf-8")
        ...
        self.assertIn("item.rule_key == 'metadata-identifiers-missing'", template)
        self.assertIn("'/titles/' ~ item.title_id ~ '/identity'", template)
        self.assertIn(".workspace-drawer-scrim:hover", template)
